"""
Multi-label Classification with BERT - Window Size Comparison
Compares different time window sizes (5s, 10s, 20s) for affective state prediction
"""
...
def prepare_multilabel_data(file_path, file_type='csv'):
    """Prepare data with multi-label encoding"""
    ...
    print(f"After removing rows with no labels: {df.shape}")
    ...
    all_labels = ['Optimistic', 'Curious', 'Confused', 'Conflicted', 
                  'Surprised', 'Disengaged', 'Frustrated', 'Engaged']
    
    # Create unique identifier for each time window
    df['window_id'] = df['timestamp'].astype(str) + '_' + df['participantID'].astype(str)
    
    # Group by window and aggregate labels
    def aggregate_labels(group):
        # Get all unique labels for this window
        labels_list = []
        for labels_str in group['labels'].dropna():
            labels_list.extend([l.strip() for l in str(labels_str).split(',')])
    ...
    # Aggregate by window
    df_agg = df.groupby('window_id', as_index=False).apply(aggregate_labels)
    df_agg = df_agg.reset_index(drop=True)
    
    print(f"\nAfter aggregating by window: {df_agg.shape[0]} unique windows")
    ...
    for label in all_labels:
        ...
        print(f"  {label}: {count} ({count/len(df_agg)*100:.1f}%)")
...
def prepare_features(X_prosody, transcripts, groups, modality='combined'):
    ...
    if modality == 'prosody':
        ...
        # Combine prosody and BERT features
...
def train_with_logo_cv(X, y, groups, label_names):
    ...
    for train_idx, test_idx in logo.split(X, y, groups):
        ...
        y_pred_proba = np.array([estimator.predict_proba(X_test_scaled)[:, 1] 
...
def save_results(results_all, label_names, window_size, output_dir='results'):
    ...
    output_file = os.path.join(output_dir, f'window_{window_size}_comparison_{timestamp}.csv')
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python multilabel_window_comparison.py <input_file> [window_size]")
        print("Example: python multilabel_window_comparison.py reports_agg_10s_windows_transcripts.csv 10s")
        sys.exit(1)
    ...
    print(f"\n{'='*60}")
    print(f"Multi-label Affective State Prediction")
    ...
    print(f"\n\n{'='*60}")
    print(f"FINAL COMPARISON - Window Size: {window_size}")
    print(f"{'='*60}")
